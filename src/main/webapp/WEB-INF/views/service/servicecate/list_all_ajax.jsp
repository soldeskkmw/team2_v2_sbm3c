...
<script type="text/javascript">
  ...
  // 수정
  function read_update_ajax(servicecateno) {
	  $('#panel_create').hide();
	  $('#panel_update').show();
	  $('#panel_delete').hide();
    ...
    var params = "";
    params = 'servicecateno=' + servicecateno; // 공백이 값으로 있으면 안됨.
    
     // params = $('#frm').serialize(); // 직렬화, 폼의 데이터를 키와 값의 구조로 조합
    // alert('params: ' + params);
    // return;
    $.ajax({
      url: '/service/servicecate/read_ajax_json.do',
      type: 'get',  // get, post
      ...
      success: function(rdata) { // 응답이 온경우
        ...
        // frm폼의 id가 servicecateno인 태그에 rdata json 객체의 servicecateno 변수의 값을 저장 
        $('#servicecateno', frm).val(rdata.servicecateno);
        $('#servicetype_content', frm).val(rdata.servicetype_content);
        $('#cnt', frm).val(rdata.cnt);
        $('#order_index', frm).val(rdata.order_index);
    ...
    $('#panel_update_animation').html("<img src='/cate/images/ani01.gif' style='width: 3%;'>");
    $('#panel_update_animation').show(); // 숨겨진 태그의 출력
    	  
	}

  // 삭제
  function read_delete_ajax(servicecateno) {
    ...
    // return;
    $.ajax({
      url: '/service/servicecate/read_ajax_json_fk.do',
      type: 'get',  // get, post
      cache: false, // 응답 결과 임시 저장 취소
      async: true,  // true: 비동기 통신
      dataType: 'json', // 응답 형식: json, html, xml...
      ...
      success: function(rdata) { // 응답이 온경우
        ...
        let msg ="";
        if (rdata.count_by_servicecateno == 0) {
        	msg = "카테고리를 삭제하면 복구 할 수 없습니다.";
          $('#btn_submit', frm).show(); // 삭제 버튼 출력
        } else {
          msg = "「" + rdata.servicetype_content + "」에 관련된 자료가 " + rdata.count_by_servicecateno + "건이 발견되었습니다.<br>";
          msg += "관련자료를 모두 삭제해야 현재 카테고리 「" + rdata.servicetype_content + "」 를 삭제 할 수 있습니다.";
        ...
        // frm폼의 id가 servicecateno인 태그에 rdata json 객체의 servicecateno 변수의 값을 저장 
        $('#servicecateno', frm).val(rdata.servicecateno);
        $('#frm_delete_name').html('카테고리: ' + rdata.servicetype_content);
    ...
    $('#panel_delete_animation').html("<img src='/cate/images/ani01.gif' style='width: 3%;'>");
...
<DIV class='title_line'>문의 카테고리</DIV>
...
<DIV class='content_body'>
  ...
  <DIV id='panel_update' style='display: none; padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    <FORM name='frm_update' id='frm_update' method='POST' action='./read_update.do'>
      <input type="hidden" name="servicecateno" id="servicecateno" value="${ServiceCateVO.servicecateno}">
  ...
  <DIV id='panel_delete' style='display: none; padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    ...
    <FORM name='frm_delete' id='frm_delete' method='POST' action='./read_delete.do'>
      <!-- 삭제할 글 번호 -->
      <input type='hidden' name='servicecateno' id='servicecateno' value="">
  ...
  <TABLE class='table table-hover'>
    ...
    <tbody>
    <c:forEach var="ServiceCateVO" items="${list }">
      ...
      <c:set var="cnt" value="${ServiceCateVO.cnt }" />
      <c:set var="order_index" value="${ServiceCateVO.order_index }" />
      <c:set var="visible" value="${ServiceCateVO.visible }" />

      <TR>
        <TD class="td_bs">${order_index}</TD>
        <TD class="td_bs_left"><A href="/service/customer_post/list_all.do?servicecateno=${servicecateno }">${servicetype_content }</A></TD>
        <TD class="td_bs">${cnt }</TD>
        <TD class="td_bs">
          <A href="/service/customer_post/create.do?servicecateno=${servicecateno}" title="${name } 등록"><IMG src="/cate/images/append.png" class="icon"></A>
          <A href="javascript: read_update_ajax(${servicecateno })" title="${name } 수정"><IMG src="/cate/images/update.png" class="icon"></A>
          <A href="javascript: read_delete_ajax(${servicecateno})" title="${name } 삭제"><IMG src="/cate/images/delete.png" class="icon"></A>
          <A href="./update_order_index_up.do?servicecateno=${servicecateno}" title="출력 순서 올림"><IMG src="/cate/images/up.png" class="icon"></A>
          <A href="./update_order_index_down.do?servicecateno=${servicecateno}" title="출력 순서 내림"><IMG src="/cate/images/down.png" class="icon"></A>
          
          <c:choose>
            <c:when test="${visible == 'Y' }">
              <A href="./update_visible_n.do?servicecateno=${servicecateno }" title="출력 모드 N로 변경"><IMG src="/cate/images/show.png" class="icon"></A>
            </c:when>
            <c:when test="${visible == 'N' }">
              <A href="./update_visible_y.do?servicecateno=${servicecateno }" title="출력 모드 Y로 변경"><IMG src="/cate/images/hide.png" class="icon"></A>            </c:when>
          </c:choose>
