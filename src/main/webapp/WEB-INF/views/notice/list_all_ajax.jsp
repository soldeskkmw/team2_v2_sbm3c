...
<script type="text/javascript">
  ...
  // 수정
  function read_update_ajax(noticeno) {
	  $('#panel_create').hide();
	  $('#panel_update').show();
    ...
    $.ajax({
      ...
      success: function(rdata) { // 응답이 온경우
        ...
        // frm폼의 id가 noticeno인 태그에 rdata json 객체의 noticeno 변수의 값을 저장 
        $('#noticeno', frm).val(rdata.noticeno);
        $('#cnt', frm).val(rdata.cnt);
        // frm폼의 id가 name인 태그에 rdata json 객체의 name 변수의 값을 저장 
        $('#noticetitle', frm).val(rdata.noticetitle);
    ...
    $('#panel_update_animation').html("<img src='/notice/images/ani01.gif' style='width: 3%;'>");
    $('#panel_update_animation').show(); // 숨겨진 태그의 출력
    	  
	}

  // 삭제
  function read_delete_ajax(noticeno) {
    ...
    // return;
    $.ajax({
      url: '/notice/read_ajax_json_fk.do',
      type: 'get',  // get, post
      cache: false, // 응답 결과 임시 저장 취소
      async: true,  // true: 비동기 통신
      dataType: 'json', // 응답 형식: json, html, xml...
      ...
      success: function(rdata) { // 응답이 온경우
        ...
        let msg ="";
        if (rdata.count_by_noticeno == 0) {
        	msg = "카테고리를 삭제하면 복구 할 수 없습니다.";
          $('#btn_submit', frm).show(); // 삭제 버튼 출력
        } else {
          msg = "「" + rdata.name + "」에 관련된 자료가 " + rdata. count_by_noticeno + "건이 발견되었습니다.<br>";
    ...
    $('#panel_delete_animation').html("<img src='/notice/images/ani01.gif' style='width: 3%;'>");
...
<DIV class='content_body'>
  ...
  <DIV id='panel_update' style='display: none; padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    <FORM name='frm_update' id='frm_update' method='POST' action='./read_update.do'>
      ...
      <input type="hidden" name="cnt" id="cnt" value="${noticeVO.cnt }">
  ...
  <TABLE class='table table-hover'>
    ...
    <c:forEach var="noticeVO" items="${list }">
      ...
      <c:set var="udate" value="${noticeVO.udate.substring(0, 16) }" />
      ...
      <TR>
        ...
        <TD class="td_bs_left"><A href="/contents/list_by_noticeno_search_paging.do?noticeno=${noticeno }">${name }</A></TD>
        <TD class="td_bs">${cnt }</TD>
        <TD class="td_bs">${rdate}</TD>
        <TD class="td_bs">${udate }</TD>
        <TD class="td_bs">
          <A href="../notice/notice_create.do?noticeno=${noticeno}" title="${noticetitle } 등록"><IMG src="/notice/images/append.png" class="icon"></A>
          <A href="javascript: read_update_ajax(${noticeno })" title="${name } 수정"><IMG src="/notice/images/update.png" class="icon"></A>
          <A href="javascript: read_delete_ajax(${noticeno})" title="${name } 삭제"><IMG src="/notice/images/delete.png" class="icon"></A>
