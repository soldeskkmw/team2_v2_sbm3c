...
<script type="text/javascript">
  ...
  function read_update_ajax(cateno) {
    ...
    $.ajax({
      ...
      success: function(rdata) { // 응답이 온경우
        ...
        $('#catename', frm).val(rdata.catename);
    ...
    $('#panel_update_animation').html("<img src='/cate/images/ani02.gif' style='width: 3%;'>");
  ...
  function read_delete_ajax(cateno) {
    ...
    $('#frm_delete_catename').html('카테고리: '); // 카테고리 이름 출력
    ...
    $.ajax({
      ...
      success: function(rdata) { // 응답이 온경우
        ...
        if (rdata.count_by_cateno == 0) {
          ...
          msg = "「" + rdata.catename + "」에 관련된 자료가 " + rdata. count_by_cateno + "건이 발견되었습니다.<br>";
          msg += "관련자료를 모두 삭제해야 현재 카테고리 「" + rdata.catename + "」 를 삭제 할 수 있습니다.";
        ...
        // frm폼의 id가 cateno인 태그에 rdata json 객체의 cateno 변수의 값을 저장 
        $('#cateno', frm).val(rdata.cateno);
        $('#frm_delete_catename').html('카테고리: ' + rdata.catename);
    ...
    $('#panel_delete_animation').html("<img src='/cate/images/ani02.gif' style='width: 3%;'>");
...
<DIV class='content_body'>
  ...
  <DIV id='panel_create' style='padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    <FORM name='frm_create' id='frm_create' method='POST' action='./create.do'>
      <label>카테고리 이름</label>
      <input type='text' name='catename' id='catename' value='' required="required" style='width: 25%;' autofocus="autofocus">
  ...
  <TABLE class='table table-hover'>
    ...
    <c:forEach var="cateVO" items="${list }">
      ...
      <TR>
        ...
        <TD class="td_bs"><A href="/post/list_by_cateno_search_paging.do?cateno=${cateno }">${catename }</A></TD>
