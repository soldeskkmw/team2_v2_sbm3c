...
<script type="text/javascript">
  ...
  function send() {
    if ($('#new_adminpasswd').val() != $('#new_adminpasswd2').val()) {
      ...
      $('#btn_close').attr("data-focus", "new_adminpasswd"); // 모달창에서 닫기 버튼 클릭시 focus를 이동할 태그 설정
      
      return false; // submit 중지
    }

    if ($('#current_adminpasswd').val() == $('#new_adminpasswd').val()) {
        msg = '· 현재 패스워드와 일치합니다.<br>';
        msg += "· 변경 패스워드를 다시 입력해주세요.<br>"; 
        ...
        $('#btn_close').attr("data-focus", "current_adminpasswd"); // 모달창에서 닫기 버튼 클릭시 focus를 이동할 태그 설정
  ...
  <DIV class='content_body'>
    ...
    <FORM name='frm' id='frm' method='POST' action='./passwd_update.do'>
      <div class="form_input">
        <label>현재 패스워드</label>    
        <input type='password' class="form-control" name='current_adminpasswd' 
                    id='current_adminpasswd' value='' required="required" style='width: 70%;' placeholder="현재 패스워드">
      </div>   
                      
      <div class="form_input">
        <label>새로운 패스워드</label>    
        <input type='password' class="form-control" name='new_adminpasswd' 
                  id='new_adminpasswd' value='' required="required" style='width: 70%;' placeholder="새로운 패스워드">
      </div>   
   
      <div class="form_input">
        <label>새로운 패스워드 확인</label>    
        <input type='password' class="form-control" name='new_adminpasswd2' 
                  id='new_adminpasswd2' value='' required="required" style='width: 70%;' placeholder="패스워드">
      </div>   
