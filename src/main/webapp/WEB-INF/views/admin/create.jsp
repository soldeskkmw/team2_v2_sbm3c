...
<script type="text/javascript">
	...
	function checkID() {
	  ...
	  let adminid = $('#adminid', frm).val(); // frm 폼에서 id가 'id'인 태그 검색
	  let params = '';
	  let msg = '';
	  ...
	  if ($.trim(adminid).length == 0) { // id를 입력받지 않은 경우
	    ...
	    $('#btn_close').attr("data-focus", "adminid");  // 닫기 버튼 클릭시 id 입력으로 focus 이동
	    ...
	    $.ajax({
	      ...
	      success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우: {"cnt":1}
	        ...
	        if (rdata.cnt > 0) {
	          $('#modal_content').attr('class', 'alert alert-danger'); // Bootstrap CSS 변경
	          msg = "이미 사용중인 ID 입니다.";
	          $('#btn_close').attr("data-focus", "adminid");  // id 입력으로 focus 이동
	        } else {
	          $('#modal_content').attr('class', 'alert alert-success'); // Bootstrap CSS 변경
	          msg = "사용 가능한 ID 입니다.";
	          $('#btn_close').attr("data-focus", "adminpasswd");  // passwd 입력으로 focus 이동
  ...
  function send() { // 회원 가입 처리
    let adminid = $('#adminid').val(); // 태그의 아이디가 'id'인 태그의 값
	  if ($.trim(adminid).length == 0) { // id를 입력받지 않은 경우
	    msg = '· ID를 입력하세요.<br>· ID 입력은 필수 입니다.<br>· ID는 3자이상 권장합니다.';
    ...
    if ($('#adminpasswd').val() != $('#adminpasswd2').val()) {
      ...
      $('#modal_content').html(msg);  // 내용
      $('#btn_close').attr('data-focus', 'adminpasswd');
      $('#modal_panel').modal();         // 다이얼로그 출력
...
<c:import url="/menu/top.do" />
  ...
  <DIV class='title_line' style="font-size:1.5rem;">관리자 회원 가입</DIV>
  ...
  <FORM name='frm' id='frm' method='POST' action='./create.do'>
     ...
     <div class="form_input" style="text-align:left;">
      <input type='text' class="form-control" name='adminid' id='adminid' value='' required="required" style='width: 30%; display:inline-block;' placeholder="아이디" autofocus="autofocus">
      <button type='button' id="btn_checkID" onclick="checkID()" style="display:inline-block;" class="btn btn-secondary">중복확인</button>
    </div> 
    
    <div class="form_input">
      <input type='password' class="form-control" name='adminpasswd' id='adminpasswd' value='' required="required" style='width: 30%;' placeholder="패스워드">
    </div>   

    <div class="form_input">
      <input type='password' class="form-control" name='adminpasswd2' id='adminpasswd2' value='' required="required" style='width: 30%;' placeholder="패스워드 확인">
    </div>   
    ...
    <div class="form_input">
     ...
     <div class="form_input">
      성별 : 
      <input type="radio" id = "M" name="admingender" value="M" required="required"> 남
      <input type="radio" id = "W" name="admingender" value="W" required="required"> 여
      <input type='text' class="form-control" name='adminage' id='adminage' value='' required="required" style='width: 10%;' placeholder="나이">
    </div>   
    ...
    <div class="form_input">
      <input type='text' class="form-control" name='address2' id='address2' 
                value='' style='width: 100%;' placeholder="상세 주소">
    </div>   
