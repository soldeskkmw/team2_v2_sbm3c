...
function setFocus() {  // focus 이동
  ...
  function send() { // 회원 가입 처리
	    let memberid = $('#memberid').val().trim(); // 태그의 아이디가 'id'인 태그의 값
      if (memberid.length == 0) { // id를 입력받지 않은 경우
        msg = '· 아이디를 입력하세요.<br>· 아이디 입력은 필수입니다.';
        
        $('#modal_content').attr('class', 'alert alert-danger'); // Bootstrap CSS 변경
        $('#modal_title').html('아이디 입력 누락'); // 제목 
        $('#modal_content').html(msg);        // 내용
        $('#btn_close').attr("data-focus", "memberid");  // 닫기 버튼 클릭시 tel 입력으로 focus 이동
  ...
  <DIV class='title_line' style="font-size:1.5rem;">인증번호 받는 email 입력</DIV>
    ...
    <DIV class='content_body'>
      <DIV style='width: 60%; margin: 0px auto;'>
           <form name="emailForm" id="emailForm" action="./send_passwd.do" method="POST">
