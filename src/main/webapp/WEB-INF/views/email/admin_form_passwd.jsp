...
function setFocus() {  // focus 이동
  ...
  function send() { // 회원 가입 처리
	    let adminid = $('#adminid').val().trim(); // 태그의 아이디가 'id'인 태그의 값
      if (adminid.length == 0) { // id를 입력받지 않은 경우
        msg = '· 아이디를 입력하세요.<br>· 아이디 입력은 필수입니다.';
        
        $('#modal_content').attr('class', 'alert alert-danger'); // Bootstrap CSS 변경
        $('#modal_title').html('아이디 입력 누락'); // 제목 
        $('#modal_content').html(msg);        // 내용
        $('#btn_close').attr("data-focus", "adminid");  // 닫기 버튼 클릭시 tel 입력으로 focus 이동
  ...
  <DIV class='title_line' style="font-size:1.5rem;">인증번호 받는 email 입력</DIV>
    ...
    <DIV class='content_body'>
      <DIV style='width: 60%; margin: 0px auto;'>
           <form name="emailForm" id="emailForm" action="./admin_send_passwd.do" method="POST">
             <!-- 정상적으로 문자가 전송되고 나서 이동할 주소 -->
            <br>
              전송메세지<br>
              <textarea name="msg" cols="30" rows="2" style="width: 500px; margin-top:7px; margin-bottom:8px;" >[www.goingshare.co.kr] [******]을 인증번호란에 입력해주세요.</textarea>
              <br>
              <div style="text-align:left;">
              아 이 디 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
              <input type="text" name="memberid" id="memberid" value='' required="required" style="margin-bottom:12px; display:inline-block;">
              <br>받는 이메일&nbsp;&nbsp;<input type="text" name="receiver" id="receiver" value='' required="required"  style="display:inline-block;"> <br>
               <!-- SMS 서비스를 가입한 기업의 관리자 전화번호 -->
              <br>
              </div>
              
              <button type="button" id='btn_send' onclick="send()" style="display:inline-block; margin-right:150px; float:right; margin-bottom:10px;"class="btn btn-secondary">전송</button>
